<a name="es"></a>
<p align="right"><a href="#en">🇬🇧 English version ↓</a></p>

# Rodrigo Valladares

**Full Stack AI Engineer** — TypeScript · Node.js · React · IA aplicada a negocio
Antofagasta, Chile 🇨🇱

[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rodrigo-valladares-73271a19b/)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:rodrigo.valladares.dev@gmail.com)

---

## Perfil Profesional

Desarrollador Full Stack con 6 años de experiencia construyendo aplicaciones web modernas en entornos de alta demanda, con foco creciente en **inteligencia artificial aplicada a negocio**. He trabajado en plataformas de e-commerce a escala nacional e implementado sistemas de IA en producción: desde agentes autónomos hasta flujos de clasificación y automatización de procesos.

Combino sólida base full stack con visión arquitectónica para integrar IA donde genera valor real. Construyo software robusto, mantenible y orientado al negocio.

---

## Experiencia

### 🏢 Falabella *(mar. 2023 – abr. 2026 · 3 años · Híbrido)*
**Desarrollador de Software** — Desarrollo de soluciones para una de las mayores empresas de retail de Latinoamérica en entorno ágil (Scrum).

- Diseñé e implementé un **sistema de ranking** para automatizar validaciones de negocio, reduciendo la intervención manual en procesos críticos
- Lideré cambios de stack tecnológico con **mejoras significativas de performance**
- Implementé **IA en la resolución de tickets de soporte**, optimizando tiempos de respuesta del equipo
- Desarrollé un sistema de **clasificación de imágenes con IA** como primera instancia en flujos de negocio
- Dicté charlas internas sobre IA y tendencias del sector al equipo de desarrollo
- *Stack: TypeScript, JavaScript, React, Node.js, APIs REST*

### 🔗 Linkify.CL *(ago. 2021 – mar. 2023 · 1 año 8 meses)*
**Developer** — Desarrollo de integraciones y herramientas internas para plataforma de acortado de links y análisis de tráfico.

- Migré la arquitectura a un **modelo basado en colas**, mejorando eficiencia y escalabilidad
- Lideré el **rediseño completo** de la aplicación (UX/UI + arquitectura frontend)
- Implementé APIs con **Apollo GraphQL** y arquitectura de microservicios
- *Stack: React, TypeScript, Node.js, Express.js, GraphQL*

### 🏢 ABZ *(oct. 2020 – ago. 2021 · 11 meses)*
**Desarrollador** — Desarrollo de software en arquitectura de microservicios.
*Stack: PHP, Node.js, Express.js, MySQL*

---

## Stack Tecnológico

**Frontend**

![TypeScript](https://img.shields.io/badge/TypeScript-%23007ACC.svg?style=flat-square&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-%2320232a.svg?style=flat-square&logo=react&logoColor=%2361DAFB)
![Angular](https://img.shields.io/badge/Angular-%23DD0031.svg?style=flat-square&logo=angular&logoColor=white)
![Vue.js](https://img.shields.io/badge/Vue.js-%2335495e.svg?style=flat-square&logo=vuedotjs&logoColor=%234FC08D)
![Next.js](https://img.shields.io/badge/Next.js-black?style=flat-square&logo=next.js&logoColor=white)

**Backend**

![Node.js](https://img.shields.io/badge/Node.js-6DA55F?style=flat-square&logo=node.js&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-%23E0234E.svg?style=flat-square&logo=nestjs&logoColor=white)
![Python](https://img.shields.io/badge/Python-3670A0?style=flat-square&logo=python&logoColor=ffdd54)
![Java](https://img.shields.io/badge/Java-%23ED8B00.svg?style=flat-square&logo=openjdk&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-%23777BB4.svg?style=flat-square&logo=php&logoColor=white)

**Bases de datos**

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-%23316192.svg?style=flat-square&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1.svg?style=flat-square&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-%234ea94b.svg?style=flat-square&logo=mongodb&logoColor=white)

**IA & Automatización**

![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat-square&logo=openai&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=flat-square&logo=n8n&logoColor=white)
![Google Cloud](https://img.shields.io/badge/Google%20Cloud-%234285F4.svg?style=flat-square&logo=google-cloud&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-%230072C6.svg?style=flat-square&logo=microsoftazure&logoColor=white)

**DevOps & Tools**

![Git](https://img.shields.io/badge/Git-%23F05033.svg?style=flat-square&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-%230db7ed.svg?style=flat-square&logo=docker&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-%23121011.svg?style=flat-square&logo=github&logoColor=white)
![GitLab](https://img.shields.io/badge/GitLab-%23181717.svg?style=flat-square&logo=gitlab&logoColor=white)

---

## Proyectos

| Proyecto | Descripción | Stack |
|----------|-------------|-------|
| **Sistema de Agentes Autónomos** | Arquitectura multi-agente para resolución de tickets sin intervención humana | Claude API, Python, n8n |
| **Command Center** | Sistema de contexto progresivo: los agentes aprenden en cada interacción | Claude API, Notion, n8n |
| [city-activities-front](https://github.com/rodrvc/city-activities-front) | Plataforma de actividades urbanas | TypeScript, Vue |
| [city-activities-api](https://github.com/rodrvc/city-activities-api) | API REST para la plataforma de actividades | TypeScript, Node.js |
| [tech-blog](https://github.com/rodrvc/tech-blog) | Blog técnico sobre IA, herramientas y desarrollo | Astro |
| [SSE-VS-GRAPHQL](https://github.com/rodrvc/SSE-VS-GRAPHQL) | Comparativa técnica de tecnologías real-time | JavaScript |

---

## Educación

**Ingeniería en Informática** — INACAP | 2018 – 2021
Miembro del Club de Programación de Antofagasta · 3er lugar en "Los Genios No Duermen"

---

## Certificaciones

- **Foundations of AR** — Meta | nov. 2024

---

*Abierto a nuevas oportunidades. Puedes escribirme por [LinkedIn](https://www.linkedin.com/in/rodrigo-valladares-73271a19b/) o [email](mailto:rodrigo.valladares.dev@gmail.com).*

---
---

<a name="en"></a>
<p align="right"><a href="#es">🇪🇸 Versión en español ↑</a></p>

# Rodrigo Valladares

**Full Stack AI Engineer** — TypeScript · Node.js · React · Applied AI
Antofagasta, Chile 🇨🇱

[![LinkedIn](https://img.shields.io/badge/LinkedIn-%230077B5.svg?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/rodrigo-valladares-73271a19b/)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:rodrigo.valladares.dev@gmail.com)

---

## Professional Profile

Full Stack Developer with 6 years of experience building modern web applications in high-demand environments, with a growing focus on **applied AI for business**. I have worked on national-scale e-commerce platforms and deployed AI systems in production: from autonomous agents to image classification pipelines and workflow automation.

I combine a strong full stack foundation with architectural vision to integrate AI where it creates real value. I build robust, maintainable, business-oriented software.

---

## Experience

### 🏢 Falabella *(Mar. 2023 – Apr. 2026 · 3 years · Hybrid)*
**Software Developer** — Building solutions for one of the largest retail companies in Latin America in an agile (Scrum) environment.

- Designed and implemented a **ranking system** to automate business validations, reducing manual intervention in critical processes
- Led tech stack changes that resulted in **significant performance improvements**
- Implemented **AI-powered ticket resolution**, optimizing team response times
- Built an **AI image classification system** as a first-instance filter in business workflows
- Delivered internal talks on AI and industry trends to the development team
- *Stack: TypeScript, JavaScript, React, Node.js, REST APIs*

### 🔗 Linkify.CL *(Aug. 2021 – Mar. 2023 · 1 year 8 months)*
**Developer** — Developing integrations and internal tooling for a link shortening and traffic analytics platform.

- Migrated the system architecture to a **queue-based model**, significantly improving efficiency and scalability
- Led a **full redesign** of the application — UX/UI and frontend architecture
- Built APIs using **Apollo GraphQL** and microservices architecture
- *Stack: React, TypeScript, Node.js, Express.js, GraphQL*

### 🏢 ABZ *(Oct. 2020 – Aug. 2021 · 11 months)*
**Developer** — Software development in a microservices architecture.
*Stack: PHP, Node.js, Express.js, MySQL*

---

## Tech Stack

**Frontend**

![TypeScript](https://img.shields.io/badge/TypeScript-%23007ACC.svg?style=flat-square&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-%2320232a.svg?style=flat-square&logo=react&logoColor=%2361DAFB)
![Angular](https://img.shields.io/badge/Angular-%23DD0031.svg?style=flat-square&logo=angular&logoColor=white)
![Vue.js](https://img.shields.io/badge/Vue.js-%2335495e.svg?style=flat-square&logo=vuedotjs&logoColor=%234FC08D)
![Next.js](https://img.shields.io/badge/Next.js-black?style=flat-square&logo=next.js&logoColor=white)

**Backend**

![Node.js](https://img.shields.io/badge/Node.js-6DA55F?style=flat-square&logo=node.js&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-%23E0234E.svg?style=flat-square&logo=nestjs&logoColor=white)
![Python](https://img.shields.io/badge/Python-3670A0?style=flat-square&logo=python&logoColor=ffdd54)
![Java](https://img.shields.io/badge/Java-%23ED8B00.svg?style=flat-square&logo=openjdk&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-%23777BB4.svg?style=flat-square&logo=php&logoColor=white)

**Databases**

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-%23316192.svg?style=flat-square&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1.svg?style=flat-square&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-%234ea94b.svg?style=flat-square&logo=mongodb&logoColor=white)

**AI & Automation**

![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat-square&logo=openai&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-EA4B71?style=flat-square&logo=n8n&logoColor=white)
![Google Cloud](https://img.shields.io/badge/Google%20Cloud-%234285F4.svg?style=flat-square&logo=google-cloud&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-%230072C6.svg?style=flat-square&logo=microsoftazure&logoColor=white)

**DevOps & Tools**

![Git](https://img.shields.io/badge/Git-%23F05033.svg?style=flat-square&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-%230db7ed.svg?style=flat-square&logo=docker&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-%23121011.svg?style=flat-square&logo=github&logoColor=white)
![GitLab](https://img.shields.io/badge/GitLab-%23181717.svg?style=flat-square&logo=gitlab&logoColor=white)

---

## Projects

| Project | Description | Stack |
|---------|-------------|-------|
| **Autonomous Agents System** | Multi-agent architecture for ticket resolution without human intervention | Claude API, Python, n8n |
| **Command Center** | Progressive context system: agents learn from each interaction | Claude API, Notion, n8n |
| [city-activities-front](https://github.com/rodrvc/city-activities-front) | Urban activities platform | TypeScript, Vue |
| [city-activities-api](https://github.com/rodrvc/city-activities-api) | REST API for the activities platform | TypeScript, Node.js |
| [tech-blog](https://github.com/rodrvc/tech-blog) | Tech blog on AI, tooling & development | Astro |
| [SSE-VS-GRAPHQL](https://github.com/rodrvc/SSE-VS-GRAPHQL) | Technical comparison of real-time technologies | JavaScript |

---

## Education

**Software Engineering** — INACAP | 2018 – 2021
Member of the Antofagasta Programming Club · 3rd place at "Los Genios No Duermen" academic competition

---

## Certifications

- **Foundations of AR** — Meta | Nov. 2024

---

*Open to new opportunities. Feel free to reach out via [LinkedIn](https://www.linkedin.com/in/rodrigo-valladares-73271a19b/) or [email](mailto:rodrigo.valladares.dev@gmail.com).*
